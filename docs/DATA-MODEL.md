...
assets: {
  [sha256]: Y.Map { w, h, mime, size, origName, addedBy, addedAt,
                    duration?, pages?, poster?, transcript?, markdown? }
}
```
...
`poster` and `transcript` are hashes of *other assets* — the still that stands for a film (T-270) and the sidecar that came in beside a recording (T-287). They are here rather than derived because neither can be recovered on the far side: a still is a decode of bytes a peer may never hold, and a transcript is a second file that existed next to the first on somebody's disk and cannot be derived from anything at all.

**And `markdown` is the one key on this record that a filename decided** (T-345, Q-324, D-65). Everything else here comes from the bytes, which is the rule the whole ingest story is built on — but a markdown file has no signature and every text file is valid markdown, so there is no total content test the way there is for a transcript's cues. Measured, reading all text as markdown would alter 96% of source files: a hash beginning a line is a comment in half the languages anybody might drop. So the exception is drawn as narrowly as it goes. The **mime is still sniffed**, and both the ingest gate and `readAsset` refuse the flag on anything that is not text — a `holiday.md` holding a JPEG is a photograph. What the name is permitted to settle is not *what this file is* but *how already-settled text should be read*, and it is settled at `app/paste.ts`, the one place on this board where a filename exists at all. It has to be recorded rather than re-derived because the reader is handed a hash and an index and never a path (`src-tauri/src/document.rs`), and a peer receiving these bytes over the wire never had a name — without this the same file would be a different document on the two machines holding it. Only `true` is ever written; an absent key and a `false` read identically.

...
| Read a recording's transcript | The item menu's *Read the transcript* — only on a tape or a cassette that arrived with a `.srt` or `.vtt` beside it. The object turns up on a sheet of it, exactly as a case file does, and **nothing plays**. Not `Enter`, which already plays it: one key that sometimes starts a recording and sometimes stands a document up in front of it is worse than either |

...
**Tape is the exception, and it is the exception because it is not pushed into the board** (Q-286). A quote card's thread is *taped* to the page it came out of rather than pinned through it — you cannot push a pin through a sheet that is lying inside a folder, and the thing that actually holds a thread to paper is tape. It is a `kind` of pin in the schema, because everything else about it is a pin: a string runs to it, it drags, it cuts, and §3.4's grammar applies unchanged. What it does not do is hold the paper up, so it is not counted here. Without that exception a folder you quote from stops hanging the moment the card arrives.

**A pin arriving is not the paper moving.** The count decides what an item hangs from, and therefore what it *turns* about when a case file opens or a note lays flat — so an item that recomputed that pivot every frame would leap the moment a pin was pushed into it, to wherever turning about its own centre would have put it. Measured at 156 board units on a folder somebody was reading (T-328). A turn takes hold of its pivot when it starts and keeps it until it ends: the paper is drawn from where the turn began, not from how many pins happen to be in it this frame. Pin an open folder, cut a clipping out of a page, undo either — nothing moves but the pin.
...
| Open a link in the browser | The item menu's *Open in browser* — only on an object that stands in for a page it could not bring onto the board (T-290), which is the only kind that has a `source`. **"Open in browser" and not "Open"**, because they are different acts and the label has to say which: everything else this menu opens, opens *on the board*, and somebody who clicks the wrong one of those has lost their place in a way no other row can cause. `http` and `https` only, checked in the frontend and again in Rust — a `source` is a field in a shared document, so it is untrusted input from a peer, and handing an unknown scheme to the shell is how a link becomes an execution |
| Read a recording's transcript | The item menu's *Read the transcript* — only on a tape or a cassette that arrived with a `.srt` or `.vtt` beside it. The object turns up on a sheet of it, exactly as a case file does, and **nothing plays**. The same row reads *Close the transcript* while one is open, because the menu is where somebody who opened it will look for how to undo it. Not `Enter`, which already plays it: one key that sometimes starts a recording and sometimes stands a document up in front of it is worse than either |
...
**A transcript is set as speech, and quoted to the recording** (T-287, Q-301). A sidecar `.srt` or `.vtt` is stored as an ordinary text asset — the store judges bytes and never a name — and D-60 made a text file a case file, so without a rule here a transcript turns up on a sheet reading its own cue numbers and arrow timestamps in our own hand, the search matches on them, and a rectangle dragged over the words cuts a card carrying them. Those timings are how a *player* knows when to put a line on screen, and this board is not a player. So a text file that turns out to be **made of cues** is read as the speech alone: the question is asked of the content and not of the extension, and it is total — every block has to be a cue, because `-->` is in prose and in every HTML comment ever written, and a note with one arrow in it must not lose its untimed lines to a parser that thought it knew better.

**And what such a card cites is the recording and the moment** — `interview.mp3 12:04` — never the sidecar and a page. The `.srt` is a file nobody put on the wall and nobody thinks of as having pages, so `interview.srt p. 1` names the wrong file and gives a number that cannot be followed back to anything you can listen to. It is the third unit in D-46's one shape: a document cites a page, a film a frame, a transcript the moment its words were said. The time is **floored where a runtime is rounded**, because a runtime is a length and a citation is a place to go and listen — a line said at 15.6 seconds cited as `0:16` sends somebody to a point where it has already been said.

**A recording with no transcript says so on itself, because the search field cannot** (T-334, Q-273). `Ctrl+F` reports what it could not read **among the matches** — a filing full of scans that has nothing to do with the query is not something the field should be talking about. A recording with no words in it falls straight through that: it can never *be* a match, so it is never in the count, and the board answers "nothing found" — true of everything it can read, and false about what is on the wall. The fix is not to relax the rule, which would put the field back to reporting on the whole board; it is that a tape nobody transcribed is a fact about **the tape**. So it wears a small `NO TRANSCRIPT` sticker, the way an archive marks a reel it never had a log sheet for: on the top right of a VHS's lip, and in the one strip a compact cassette has between its J-card and its capstan cutouts. Somebody looking for a line they half-remember can see which of the recordings on the wall were never going to answer, without having to ask. It is drawn off the asset record rather than off the bytes, so it is right on a peer that holds neither the interview nor the sidecar; a case file needs none, because a document that cannot be read has a page that says why.

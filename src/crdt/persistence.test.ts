...
describe("Persistence", () => {
  ...
  describe("closing", () => {
    ...
    /**
     * The other half of the same contract, and the reason `close()` flushes at
     * all rather than simply unsubscribing: the last fifth of a second of a
     * session is work like any other, and a switch is not a licence to drop it.
     */
    it("sends what was pending rather than dropping it on the floor", async () => {
      const store = new Store();
      const { board, persistence } = await session(store);
      polaroid(board, 10, 20);
      // Un-flushed: the batch is still open when the close arrives.
      expect(store.appends).toBe(0);

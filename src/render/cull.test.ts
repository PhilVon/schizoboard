...
describe("Culler", () => {
  ...
  describe("invalidation", () => {
    ...
    /**
     * T-133, both halves: the bounds have to count the ink, and the grid has to
     * be told when the ink changes. Either one alone leaves an item unmounted
     * with part of its stroke on screen.
     */
    it("keeps an item mounted for ink that overhangs into the viewport", () => {
      // Enough items to put the culler on the grid path, which is the one that
      // can be stale: the scan re-derives every item's bounds, so it would pass
      // this whether or not the index was told the ink had changed.
      fillerFarAway();
      // Off to the right of the viewport by a long way, with nothing on it.
      put("a", 4000, 300, 100, 100);
      expect(sorted(frame())).toEqual([]);
      expect(culler.path).toBe("grid");
      ...
      scene.putStrokes("a", [
        {
          ...
          seed: 1,
          z: "a0",
          // Drawn off the left edge of the paper and most of the way back to the
          // camera — the mark is on screen even though the paper is not.
          bbox: [-3800, 0, 0, 0],
          samples: [
            { x: -3800, y: 0, pressure: 0.5 },
    ...
    it("lets the item go again when that ink is erased", () => {
      put("a", 4000, 300, 100, 100);
      scene.putStrokes("a", [
        {
          id: "s",

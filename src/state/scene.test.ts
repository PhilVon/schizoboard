...
/**
 * T-133. Ink is not clipped to the paper it was drawn on — the canvas hangs off
 * the item root precisely so a line can run off the edge — so an item's bounds
 * have to count it. The failure without this is quiet and one-sided: the paper
 * leaves the viewport, the culler unmounts it, and the part of the stroke that
 * was still on screen goes with it.
 */
describe("bounds with ink on the item", () => {
  function inked(scene: Scene, id: string, box: [number, number, number, number], size = 6): void {
    scene.putStrokes(id, [
      {
        id: `${id}-s`,
        tool: "marker",
        ...
        bbox: box,
        samples: [
          { x: box[0], y: box[1], pressure: 0.5 },
          { x: box[2], y: box[3], pressure: 0.5 },
        ],
      },
    ]);
  }

  it("reaches past the paper for a stroke drawn off the edge of it", () => {
    const scene = new Scene();
    scene.putItem(cold("a"), pose({ x: 0, y: 0, w: 100, h: 100 }));
    const paper = scene.boundsOf("a")!.maxX;

    inked(scene, "a", [0, 0, 400, 0]);
    // 400 units out on a 100-unit sheet: the mark is four sheets long and most
    // of it is on the cork.
    expect(scene.boundsOf("a")!.maxX).toBeGreaterThan(400);
    expect(paper).toBeCloseTo(50, 5);
  });

  it("counts the nib, not just the points", () => {
    const scene = new Scene();
    scene.putItem(cold("a"), pose({ w: 10, h: 10 }));
    inked(scene, "a", [0, 0, 200, 0], 40);

    // A 40-unit highlighter laid down with its centre line ending at 200 puts
    // ink a good deal further out than 200, and a box drawn round the samples
    // alone clips it.
    expect(scene.boundsOf("a")!.maxX).toBeGreaterThan(220);
  });

  it("adds nothing for ink that stays inside the paper, which is most ink", () => {
    const scene = new Scene();
    scene.putItem(cold("a"), pose({ w: 400, h: 400 }));
    inked(scene, "a", [-10, -10, 10, 10]);
    expect(scene.boundsOf("a")!.maxX).toBeCloseTo(200, 5);
  });

  it("is the same reach at every angle, because it is measured in the item's frame", () => {
    const scene = new Scene();
    scene.putItem(cold("a"), pose({ w: 10, h: 10 }));
    inked(scene, "a", [0, 0, 300, 0]);
    const flat = scene.boundsOf("a")!.maxX;

    scene.setPose("a", { rot: Math.PI / 3 });
    // A radius, not a box: a swinging item never recomputes this, and that is
    // the whole reason it is a radius.
    expect(scene.boundsOf("a")!.maxX).toBeCloseTo(flat, 5);
  });

  it("shrinks back when the ink is erased", () => {
    const scene = new Scene();
    scene.putItem(cold("a"), pose({ w: 100, h: 100 }));
    inked(scene, "a", [0, 0, 400, 0]);
    scene.putStrokes("a", []);

    // Not grow-only, unlike the bitmap in `render/ink/canvas.ts`: that one is a
    // cache eviction reclaims, and this one would keep an item mounted forever.
    expect(scene.boundsOf("a")!.maxX).toBeCloseTo(50, 5);
  });

  it("survives ink that arrives before the item it belongs to", () => {
    const scene = new Scene();
    inked(scene, "a", [0, 0, 400, 0]);
    scene.putItem(cold("a"), pose({ w: 100, h: 100 }));

    // An undo restores an item and its strokes in one transaction and nothing
    // promises the observer delivers the halves in one order.
    expect(scene.boundsOf("a")!.maxX).toBeGreaterThan(400);
  });

  it("does not carry one item's reach into the next item in the slot", () => {
    const scene = new Scene();
    scene.putItem(cold("a"), pose({ w: 100, h: 100 }));
    inked(scene, "a", [0, 0, 400, 0]);
    scene.removeItem("a");

    scene.putItem(cold("b"), pose({ w: 100, h: 100 }));
    expect(scene.boundsOf("b")!.maxX).toBeCloseTo(50, 5);
  });
});
